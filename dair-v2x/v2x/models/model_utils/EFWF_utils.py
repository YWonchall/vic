...
def get_filted_index(pointclouds, boxes,scores,n):
    # pointclouds: [M,4]
    # boxes: [N,8,3]
    # scores : [N]
...
def filt_point_by_boxes(pcd, pred,n):
    ...
    boxes = np.array(pred['boxes_3d'])
    scores = np.array(pred['scores_3d'])
    # point_cloud = o3d.geometry.PointCloud()
    # point_cloud.points = o3d.utility.Vector3dVector(pcd_array[:,:3])
    # o3d.io.write_point_cloud(folder_path+f'/source_points.pcd',point_cloud)

    retain_ind = get_filted_index(pcd_array,boxes,scores,n)
