...
def build_label_list(annos, filt, pred=False):
    result_list = []
    for i in range(len(annos["labels_3d"])):
        if superclass[annos["labels_3d"][i]] == filt:
            if pred:
                result_list.append({"box": annos["boxes_3d"][i], "score": annos["scores_3d"][i],"inf_pred": annos["inf_pred"][i],"inf_scores": annos["inf_scores"][i]})
            else:
                result_list.append({"box": annos["boxes_3d"][i], "score": annos["scores_3d"][i]})
    return result_list


def assign_gt_boxes(gt_annos, pred_annos, cla, iou_threshold, view):
    """
    Input:
    ...
    """
    gt_annos = build_label_list(gt_annos, filt=cla, pred=False)
    pred_annos = build_label_list(pred_annos, filt=cla, pred=True)
    ...
    for gt_anno in gt_annos:
        ...
        if mx_pred is not None:
            pred_annos[mx_pred]['gt_boxes']= gt_anno["box"]
